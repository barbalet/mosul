...
DERIVED_DATA_ROOT="${DERIVED_DATA_ROOT:-$ROOT_DIR/build/mosulgame-audio-smoke}"
DESTINATION="${DESTINATION:-platform=macOS}"
REPORT_PATH="${REPORT_PATH:-$ROOT_DIR/snapshots/evidence/mosul-audio-smoke.txt}"
SKIP_BUILD=0
TIMEOUT_SECONDS="${AUDIO_SMOKE_TIMEOUT_SECONDS:-60}"
...
Options:
  ...
  --derived-data-root PATH  Build output root. Defaults to build/mosulgame-audio-smoke.
  ...
  --timeout SECONDS         Fail if the app probe does not exit in time. Defaults to 60.
...
run_with_watchdog() {
  ...
  (
    ...
    if [[ ! -s "$REPORT_PATH" ]]; then
      echo "error: audio smoke probe timed out after ${timeout_seconds}s" >&2
      terminate_app
    fi
  ) &
  watchdog_pid=$!
...
run_with_watchdog "$TIMEOUT_SECONDS" \
  ...
  --audio-smoke-report "$REPORT_PATH"

if [[ ! -s "$REPORT_PATH" ]]; then
  echo "error: MosulGame did not write audio smoke report: $REPORT_PATH" >&2
  exit 1
fi

if ! grep -q '^ok=true$' "$REPORT_PATH"; then
  echo "error: MosulGame audio smoke failed: $REPORT_PATH" >&2
  cat "$REPORT_PATH" >&2
  exit 1
fi
