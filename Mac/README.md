...
It renders the runtime map overview from `modernerKrieg/assets/mosul/runtime/`, resolves C-core unit glyphs through `modernerKrieg/assets/mosul/runtime/sprites/manifest.json`, draws the matching runtime PNG sprites, overlays objectives/civilians/contact reports, and can step the simulation or run the deterministic AI loop.
